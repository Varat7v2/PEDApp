...
class myPYSPICE(object):
    ...
    def simulate_ckt(self, netlist_dict, converter):
        ...
        PySpice_netlist = open('{}/{}_{}'.format(myconfig.DATA, self.title, converter), 'w')
        ...
        duty_cycle = random.uniform(0.6, 0.6)
        fs = random.randint(50, 50)@u_kHz        # Switching frequency
        Ts = fs.period
        Ton = duty_cycle * Ts                    # T_ON

        print('[INFO]: Dutycycle = ', duty_cycle)
        print('[INFO]: Switching frequency = ', fs)
        ...
        for k,v in netlist_dict.items():
            ...
            if myconfig.DEBUG_PEDAPP:
                ...
                print('\t\t{} {}: {} {}'.format(COMPONENT_COUNT, component, str(terminal1), str(terminal2)))
            ...
            S_value = random.randint(48,  48)               # value in Volts(V)
            R_value = round(random.uniform(4, 4), 2)        # value in ohms (Ohm)
            L_value = random.randint(120, 120)              # value in micro henry (uH)
            C_value = random.randint(10, 10)              # value in micro farads (uF)
            ...
            if component == 'source':
                ...
                # circuit.V('branch_V', 'xv{}'.format(S_count), terminal2, 0@u_V)
